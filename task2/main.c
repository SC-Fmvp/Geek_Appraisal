...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]>0)
        {
            arr_small[count2++] = arr_big[i];	/*将字符串中出现的字母次数放入数组arr_small中*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*让左右各叶节点录入计算路径长度*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*当叶节点全部录入后跳出循环*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*找到最小的min，当找到最小的min不再进入if*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    {
    	if(huffman[e][3]==-1 && huffman[e][0]==-1)
    	{
    		break;
		}
		if(huffman[e][3] < min2&&huffman[e][3] > min&&huffman[e][0] == -1)
		{
			min2 = huffman[e][3];
			ii[1] = e;}}
...
{
    int length = 0;
    if(huffman[i][0] == -1)     /*当父节点全部录入时终止程序*/
    {
        return 0;
    }
